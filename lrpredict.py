...
###对出现过反馈的用户进行预测
def predict_1(data1,data2,i,j):
    linreg1 = RidgeCV(alphas=np.logspace(-3, 2, 10))
    linreg2 =RidgeCV(alphas=np.logspace(-3, 2, 10))
    linreg3 = RidgeCV(alphas=np.logspace(-3, 2, 10))
...
#    predict_data =pd.DataFrame( ss.transform(predict_data),columns=['topic','http','@','emotion','wordsum','len_content','time_weekend','time_weekday','time_hour','panduan','forward_max','comment_max','like_max','forward_min','comment_min','like_min','forward_mean','comment_mean','like_mean','forward_more_ave_pr','comment_more_ave_pr','like_more_ave_pr'])
    predict_forward=linreg1.fit(train_data.loc[:,['topic','http','@','emotion','wordsum','len_content','time_weekend','time_weekday','time_hour','forward_max','forward_min','forward_mean','forward_more_ave_pr','max_f/l','max_c/l','min_f/l','min_c/l','mean_f/l','mean_c/l']],train_forward)
    ...
    predict_comment=linreg2.fit(train_data.loc[:,['topic','http','@','emotion','wordsum','len_content','time_weekend','time_weekday','time_hour','comment_max','comment_min','comment_mean','comment_more_ave_pr','max_f/l','max_c/l','min_f/l','min_c/l','mean_f/l','mean_c/l']],train_comment)
    ...
    predict_like=linreg3.fit(train_data.loc[:,['topic','http','@','emotion','wordsum','len_content','time_weekend','time_weekday','time_hour','like_max','like_min','like_mean','like_more_ave_pr','max_f/l','max_c/l','min_f/l','min_c/l','mean_f/l','mean_c/l']],train_like)
...
for i in range(4,20):
    data_1_=predict_1(train_data26,data_1,i,3)
